...
if (strlen($_SESSION['hbmsuid'] == 0)) {
	...
	if (isset($_POST['submit'])) {
		...
		$abs_diff = $OutDate->diff($InDate)->format("%a");

		$total_booking_amount = $rprice*$abs_diff;
		...
		if ($checkindate <  $cdate) {
			...
			$sql = "insert into tblbooking(RoomId,BookingNumber,UserID,IDType,Gender,Address,CheckinDate,CheckoutDate,Total_Booking_Amount)values(:rid,:booknum,:uid,:idtype,:gender,:address,:checkindate,:checkoutdate,:tamount)";
			...
			$query->bindParam(':tamount', $total_booking_amount, PDO::PARAM_STR);
			$query->execute();
						...
						<div class="col-md-6 contact-right">
							<form method="post">
								...
								<select type="text" value="" class="form-control" name="idtype" required="true" class="form-control">
									...
									<option value="Adhar Card">NIC</option>
